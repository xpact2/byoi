...
#####################################
resource "aws_security_group" "byoi-bastion-sg" {
  name        = "bastion-sg"
...
############################
resource "aws_security_group" "byoi-ALB-SG" {
  name        = "ALB-SG"
  description = "Allow port 80 and 443 from anywhere"
  vpc_id      = aws_vpc.byoi-vpc.id
  tags = {
    "Name" = "sg-for-byoi-alb"
  }
  ingress {
...
###################################
resource "aws_security_group" "byoi-App-SG" {
  name        = "App-SG"
  ...
  #   to_port         = 22
  #   protocol        = "tcp"
  #   security_groups = [aws_security_group.byoi-bastion-sg.id]
...
###################################
resource "aws_security_group" "byoi-DB-SG" {
  name        = "DB-SG"
  ...
  ingress {
    ...
    to_port         = 3306
    protocol        = "tcp"
    security_groups = [aws_security_group.byoi-App-SG.id]
